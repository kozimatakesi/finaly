...
ipcMain.on('readExcelFile', (e, dirPath) => {
  ...
  const careerInfo = getLineDataTitle('キャリア');
  const careerInfoArray = Object.values(careerInfo[0]);
  const UESerch = getLineDataTitle('端末');
  const UESerchArray = Object.values(UESerch[0]);
  const testInfo = getLineDataTitle('試験内容');
  const testInfoArray = Object.values(testInfo[0]);
  const bandLockInfo = getLineDataTitle('BAND');
  ...
  const scannerInfoSerch = getLineDataTitle('スキャナ機種');
  const scannerInfo = Object.values(scannerInfoSerch[0]);
  scannerInfo.shift();
  const areaInfoSerch = getLineDataTitle('測定エリア');
  const meansInfoSerch = getLineDataTitle('測定内容');
  const daysInfoSerch = getLineDataTitle('測定日');
